    ...
    if replied.video or replied.document:
        ...
        if chat_id in QUEUE:
            ...
            try:
                ...
                await m.reply_text("⌯ مفيش كول شغل يكفيف عشان اشغل\n\n» اكتب ، اطلع وحاول مره اخره ")
            except Exception as e:
                LOGS.info(f"[ERROR]: {e}")
    else:
        await m.reply_text(
...
@Client.on_message(command(["يديو", f"يد","vplay"]) & other_filters)
@check_blacklist()
...
async def video_stream(c: Client, m: Message):
    ...
    try:
        ...
        if b.status == "banned":
            try:
                await m.reply_text("⌯ المساعد محظور ، ارفع الحظر عنة واكتب ، تحديث وبعدين اكتب ، انضم حتى تقدر تشغل ")
                await remove_active_chat(chat_id)
            except BaseException:
                pass
                ...
                if data == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await loser.delete()
                            await remove_active_chat(chat_id)
                            await m.reply_text("⌯ مفيش كول اصلا يكفيف عشان اشغل\n\n⌯اكتب ، اطلع وحاول مره اخرى")
                        except NoVideoSourceFound:
                            await loser.delete()
                            await remove_active_chat(chat_id)
                            await m.reply_text("⌯ هذا المحتوى لايحتوي على صوت")
