...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply_text("⌯ تم ايقاف ...")
        except Exception as e:
            traceback.print_exc()
            await m.reply_text(f"🚫 **error:**\n\n`{e}`")
    else:
        await m.reply_text("⌯ مفيش حاجه شغله اصلا ...")


@Client.on_message(
    command(["سكت", f"pause@{BOT_USERNAME}", "vpause"]) & other_filters
)
@authorized_users_only
@check_blacklist()
...
@Client.on_message(
    command(["مل", f"resume@{BOT_USERNAME}", "vresume"]) & other_filters
)
@authorized_users_only
@check_blacklist()
...
@Client.on_message(command(["خطي" ,"لتالي"]) & other_filters)
@authorized_users_only
@check_blacklist()
...
async def unmute(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            if await is_music_playing(chat_id):
                return await m.reply_text("ℹ️ جاي التشغيل ...")
...
@Client.on_message(
    command(["ضبط", "لصوت" ,"vol"]) & other_filters
)
@authorized_users_only
@check_blacklist()
async def change_volume(c: Client, m: Message):
    if len(m.command) < 2:
        return await m.reply_text("الاستخدام: `الصوت` (`0-200`)")
...
async def cbstop(_, query: CallbackQuery):
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("⌯ مفيش حاجه شغله اصلا ...", show_alert=True)
    chat_id = query.message.chat.id
    if chat_id in QUEUE:
        try:
            await calls.leave_group_call(chat_id)
            await remove_active_chat(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("⌯ تم الايقاف ...", reply_markup=close_mark)
        except Exception as e:
            traceback.print_exc()
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=close_mark)
    else:
...
async def cbskip(_, query: CallbackQuery):
    ...
    if queue == 0:
        ...
        await query.answer("⌯ جاري الانتقال الى الاغنية الثانيه ...")
